...
# query cache and asynchronous queries will then be enabled.
...
#  "-vf 'select=eq(n\\,0)+eq(key\\,1)+gt(scene\\,0.015),loop=loop=-1:size=2,
#    trim=start_frame=1' -frames:v 1 -f image2"
...
# namespace IDs provided as a
# `String` to `Digest::UUID.uuid_v3` or `Digest::UUID.uuid_v5` method calls.
#
# See https://guides.rubyonrails.org/configuring.html#config-active-
#   support-use-rfc4122-namespaced-uuids for
# more information.
...
# `:hybrid` option.
#
# Rails transparently deserializes existing (Marshal-serialized) cookies on read
# and re-writes them in the JSON format.
#
...
# of the file.
#
# See https://guides.rubyonrails.org/action_controller_overview.html#cookies for
# more information.
...
# current collection instead of appending to it. Thus, to support submitting an
# empty collection, the `file_field` helper will render an hidden field
# `include_hidden` by default when `multiple_file_field_include_hidden` is set
# to `true`. See
...
#   config-active-support-disable-to-s-conversion for more information.
